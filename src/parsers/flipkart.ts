...
import { readXlsxObjects } from "./xlsx";
import {
...
type FlipkartRow = Record<string, string>;
...
const DOC_CATEGORY_BY_SERIES: Record<string, DocumentCategory> = {
  FAONVN: "invoice",
  LWAAKVG: "invoice",
  RAMDVE: "credit_note",
  MFAAJLK: "credit_note",
...
function toDecimal(value: string): Decimal {
  ...
  return normalized === "" || normalized === "NA" ? new Decimal(0) : new Decimal(normalized);
}

function getDocSeries(number: string): string {
...
function getBusinessGstin(row: FlipkartRow): string | undefined {
...
function getRate(row: FlipkartRow): Decimal {
  return toDecimal(row["IGST Rate"] ?? "")
    .plus(toDecimal(row["CGST Rate"] ?? ""))
    .plus(toDecimal(row["SGST Rate (or UTGST as applicable)"] ?? ""));
}

function buildSalesDocumentIssue(row: FlipkartRow): DocumentIssueRecord | null {
...
function buildCashbackDocumentIssue(row: FlipkartRow): DocumentIssueRecord | null {
  ...
  const fallbackCategory =
    (row["Document Type"] ?? "").trim() === "Debit Note" ? "debit_note" : "credit_note";
...
function buildSalesRecord(
  ...
  const pos = toGSTStateCode(
    (row["Customer's Delivery State"] ?? "").trim() ||
      (row["Customer's Billing State"] ?? "").trim()
  );
  const eventType = (row["Event Type"] ?? "").trim();
  const fallbackCategory =
    eventType === "Sale" ? "invoice" : taxableValue.greaterThanOrEqualTo(0) ? "debit_note" : "credit_note";
...
function buildCashbackRecord(
  row: FlipkartRow,
  sellerState: GSTStateCode,
  hsnByOrderItemId: Map<string, string>
): NormalizedSupplyRecord | null {
  ...
  const sgst = toDecimal(row["SGST Amount (Or UTGST as applicable)"] ?? "");
  const ctin = getBusinessGstin(row);
  const pos = toGSTStateCode((row["Customer's Delivery State"] ?? "").trim());
  const fallbackCategory =
    (row["Document Type"] ?? "").trim() === "Debit Note" ? "debit_note" : "credit_note";
...
export function parseFlipkartWorkbook(
  content: Buffer,
  options: ParseFlipkartOptions = {}
): SourceExtraction {
  const sellerState = options.sellerState ?? DEFAULT_SELLER_STATE;
  const salesRows = readXlsxObjects(content, SALES_SHEET);
  const cashbackRows = readXlsxObjects(content, CASHBACK_SHEET);
  const hsnByOrderItemId = new Map<string, string>();
